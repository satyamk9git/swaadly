...
export const scaleIngredients = async (dishName: string, servings: number, spice: string): Promise<string[]> => {
  try {
    const response = await ai.models.generateContent({
      model: "gemini-1.5-flash",
      contents: `You are a culinary expert. Scale the ingredients for "${dishName}" to serve exactly ${servings} people with a ${spice} spice level. 
...
export const classifyGroceryItem = async (input: string): Promise<Partial<GroceryItem>> => {
  try {
    const response = await ai.models.generateContent({
      model: "gemini-1.5-flash",
      contents: `Classify this grocery item into a category: "${input}". 
...
export const consolidateGroceryList = async (items: GroceryItem[]): Promise<GroceryItem[]> => {
  try {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-1.5-flash",
      contents: `Consolidate this grocery list. 
    ...
    const consolidated = JSON.parse(response.text);
...
export const parseZeptoItems = async (input: string): Promise<Array<{ name: string; quantity: string }>> => {
  try {
    const response = await ai.models.generateContent({
      model: "gemini-1.5-flash",
      contents: `Parse these grocery items from a raw list and separate the name and quantity.
...
export const generateDishImage = async (dishName: string): Promise<string> => {
  // Use a high-quality food image service that searches by keyword
  // This provides more realistic results than AI generation and is much faster
